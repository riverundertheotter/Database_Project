...
CREATE TABLE
    COMPANY (c_id INTEGER PRIMARY KEY, c_name TEXT NOT NULL);
...
CREATE TABLE
    PHONE_NUMBER (
        ...
        FOREIGN KEY (personal_id) REFERENCES COMPANY_AFFILIATE (personal_id),
        PRIMARY KEY (personal_id, phone_num)
    );
...
CREATE TABLE
    APPLICATION (
        ...
        personal_id INTEGER NOT NULL DEFAULT 0
        ...
        -- FOREIGN KEY (personal_id_potential) REFERENCES POTENTIAL_EMPLOYEE (personal_id)
        FOREIGN KEY (personal_id) REFERENCES COMPANY_AFFILIATE (personal_id) -- FIX HERE
    );

CREATE TABLE
    INTERVIEW ( -- personal_id, round_num, and app_id should be primary key
